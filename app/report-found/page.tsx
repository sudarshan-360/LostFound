...
import { ArrowLeft, Upload, X, Calendar, Phone, Mail, Camera, CheckCircle } from "lucide-react"
...
export default function ReportFoundItem() {
  ...
  const findMatchingItems = (foundItem: typeof formData) => {
    const lostItems = JSON.parse(localStorage.getItem("userReports") || "[]")
    ...
    return lostItems.filter((lostItem: any) => {
      ...
      const nameMatch = lostItem.itemName.toLowerCase().includes(foundItem.itemName.toLowerCase()) ||
                       foundItem.itemName.toLowerCase().includes(lostItem.itemName.toLowerCase())
      
      // Match by description (partial match)
      const descriptionMatch = lostItem.description.toLowerCase().includes(foundItem.description.toLowerCase()) ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditMode) {
      ...
      // Redirect to matches page to see potential lost items that match this found item
      router.push("/matches")
    }
  }
              ...
              <div className="space-y-2">
                ...
                <DatePicker
                  ...
                  onDateChange={(date) => {
                    ...
                    handleInputChange("dateFound", date ? date.toISOString().split('T')[0] : "")
                  }}
